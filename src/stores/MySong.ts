...
export const useMySong = defineStore('MySong', {
    ...
    actions: {
        ...
        async getLikeList() {
            const localStore = useLocalStore()
            const res = await reqLikeSongs({ uid: localStore.userId, timestamp: +new Date() })
            if (res.code === 200) {
        ...
        async getUserLikeAlbum() {
            const res = await reqLikeAlbum({ limit: 30, timestamp: +new Date() })
            if (res.code === 200) {
        ...
        async getUserLikeArtist() {
            const res = await reqLikeArtist({ limit: 30, timestamp: +new Date() })
            if (res.code === 200) {
        ...
        // 获取用户关注mv
        async getUserLikeMV() {
            const res = await reqLikeMV({ limit: 30, timestamp: +new Date() })
            if (res.code === 200) {
        ...
        async getUserLikeCloud() {
            const res = await reqlikeCloud({ limit: 300, offset: 0, timestamp: +new Date() })
            if (res.code === 200) {
